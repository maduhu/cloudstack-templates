...
usage() {
	echo "Usage: $0 --release|-r <trusty|precise> [options]
Options are:
 --minimal|-m
 --debootstrap-url|-u <debootstrap-mirror> (default: http://no.archive.ubuntu.com/ubuntu)
 --sources.list-mirror|-s <source-list-mirror> (default: http://no.archive.ubuntu.com/ubuntu/)
...
if [ -z "${DEB_MIRROR}" ] ; then
	DEB_MIRROR=http://no.archive.ubuntu.com/ubuntu/
fi
if [ -z "${EXTRA_PACKAGES}" ] ; then
	EXTRA_PACKAGES=bash-completion,screen,less,vim,bzip2
fi
if [ -z "${SOURCE_LIST_MIRROR}" ] ; then
	SOURCE_LIST_MIRROR=http://no.archive.ubuntu.com/ubuntu/
fi
...
FILE_NAME=ubuntu-${RELEASE}-14.04-amd64
...
deb ${SOURCE_LIST_MIRROR} precise-backports main
